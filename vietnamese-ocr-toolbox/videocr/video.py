...
class Video:
    ...
    def run_ocr(
        ...
        self.pred_frames = []
        ocr = Pipeline(self.args, self.config)
        ...
        with Capture(self.path) as v:
            ...
            for i in range(num_ocr_frames):
                if i % modulo == 0:
                    ...
                    print(ocr.start(frame))
                    predicted_frames = PredictedFrames(
                        i + ocr_start, ocr.start(frame), conf_threshold_percent
                    )
    ...
    def _generate_subtitles(self, sim_threshold: int) -> None:
        ...
        self.pred_subs = [sub for sub in self.pred_subs if len(sub.frames[0].words) > 0]
