...
def writeSlices(img_serie, series_tag_values, i, final_folder):
    ...
    image_slice.SetMetaData(
        "0020|0032",
        "\\".join(map(str, img_serie.TransformIndexToPhysicalPoint((0, 0, i)))),
...
def convert(_root, _output, _input, _type):
    ...
    for patient in tqdm(patients):
        ...
        img_path = (
            patient if _type == "PATIENT" else patient.replace("volume", "segmentation")
        )
        ...
        list(
            map(
                lambda k: writeSlices(
                    img_serie=nii_image,
                    series_tag_values=series_tag_values,
                    i=k,
                    final_folder=patient_folder,
                ),
                range(nii_image.GetDepth()),
            )
        )
