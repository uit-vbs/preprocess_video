...
class YoloLoss:
    def __init__(
        ...
        # Define criteria
        self.BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([self.cls_pw])).to(
            device
        )
        self.BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([self.obj_pw])).to(
            device
        )
        ...
        if g > 0:
            self.BCEcls, self.BCEobj = FocalLoss(self.BCEcls, g), FocalLoss(
                self.BCEobj, g
            )
        ...
        self.giou *= 3.0 / self.nl  # scale to layers
        self.cls_w *= self.nc / 80.0 * 3.0 / self.nl  # scale to classes and layers
        self.obj_w *= (1280 / 640) ** 2 * 3.0 / self.nl
