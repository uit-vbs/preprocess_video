...
def get_yolo(model_name):
    ...
    net = YoloBackbone(version_name=version_name, num_classes=1)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if version == "4":
            ...
            self.model = Yolov4(
                cfg=f"./models/yolo/configs/yolov4-{version_mode}.yaml",
                ch=3,
                nc=num_classes,
            )
        elif version == "5":
            ...
            self.model = Yolov5(
                cfg=f"./models/yolo/configs/yolov5{version_mode}.yaml",
                ch=3,
                nc=num_classes,
            )
        ...
        if pretrained_backbone_path is not None:
            ckpt = torch.load(
                pretrained_backbone_path, map_location="cpu"
            )  # load checkpoint
            try:
        ...
        self.loss_fn = YoloLoss(num_classes=num_classes, model=self.model)
