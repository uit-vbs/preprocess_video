...
class Model(nn.Module):
    def __init__(self, model_config: dict):
        ...
        assert (
            segmentation_head in segmentation_head_dict
        ), "segmentation_head must in: {}".format(segmentation_head_dict)

        backbone_model, backbone_out = (
            backbone_dict[backbone]["models"],
            backbone_dict[backbone]["out"],
        )
        self.backbone = backbone_model(pretrained=pretrained)
        self.segmentation_head = segmentation_head_dict[segmentation_head](
            backbone_out, **model_config
        )
