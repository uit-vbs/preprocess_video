...
class FPN(nn.Module):
    def __init__(self, backbone_out_channels, **kwargs):
        ...
        self.reduce_conv_c2 = nn.Sequential(
            nn.Conv2d(
                backbone_out_channels[0], conv_out, kernel_size=1, stride=1, padding=0
            ),
        ...
        self.reduce_conv_c3 = nn.Sequential(
            nn.Conv2d(
                backbone_out_channels[1], conv_out, kernel_size=1, stride=1, padding=0
            ),
            nn.BatchNorm2d(conv_out),
            nn.ReLU(inplace=inplace),
        )
        self.reduce_conv_c4 = nn.Sequential(
            nn.Conv2d(
                backbone_out_channels[2], conv_out, kernel_size=1, stride=1, padding=0
            ),
            nn.BatchNorm2d(conv_out),
            nn.ReLU(inplace=inplace),
        )
        ...
        self.reduce_conv_c5 = nn.Sequential(
            nn.Conv2d(
                backbone_out_channels[3], conv_out, kernel_size=1, stride=1, padding=0
            ),
            nn.BatchNorm2d(conv_out),
            nn.ReLU(inplace=inplace),
        )
...
class FPEM_FFM(nn.Module):
    def __init__(self, backbone_out_channels, **kwargs):
        ...
        self.out_conv = nn.Conv2d(
            in_channels=conv_out * 4, out_channels=6, kernel_size=1
        )
