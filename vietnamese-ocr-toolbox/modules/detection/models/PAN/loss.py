...
class PANLoss(nn.Module):
    ...
    def forward(self, outputs, labels, training_masks):
        ...
        loss_aggs, loss_diss = self.agg_dis_loss(
            texts, kernels, gt_texts, gt_kernels, similarity_vectors
        )
        ...
        loss_all = (
            loss_text + self.alpha * loss_kernel + self.beta * (loss_agg + loss_dis)
        )
    ...
    def agg_dis_loss(self, texts, kernels, gt_texts, gt_kernels, similarity_vectors):
        ...
        loss_aggs = []
        loss_diss = []
        for text_i, kernel_i, gt_text_i, gt_kernel_i, similarity_vector in zip(
            texts, kernels, gt_texts, gt_kernels, similarity_vectors
        ):
            ...
            for text_idx in range(1, int(text_num)):
                ...
                text_G_ki = (text_similarity_vector - G_kernel.reshape(4, 1)).norm(
                    2, dim=0
                ) - self.delta_agg
                # D(p,K_i), shape: nums
                D_text_kernel = torch.max(
                    text_G_ki,
                    torch.tensor(0, device=text_G_ki.device, dtype=torch.float),
                ).pow(2)
            ...
            if len(loss_agg_single_sample) > 0:
                ...
                loss_agg_single_sample = torch.tensor(
                    0, device=texts.device, dtype=torch.float
                )
            ...
            for G_kernel_i, G_kernel_j in itertools.combinations(G_kernel_list, 2):
                ...
                # D(K_i,K_j)
                D_kernel_ij = torch.max(
                    kernel_ij,
                    torch.tensor(0, device=kernel_ij.device, dtype=torch.float),
                ).pow(2)
            ...
            if len(G_kernel_list) > 1:
                ...
                loss_dis_single_sample = torch.tensor(
                    0, device=texts.device, dtype=torch.float
                )
