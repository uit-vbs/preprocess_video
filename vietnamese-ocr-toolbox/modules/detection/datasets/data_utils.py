...
def check_and_validate_polys(polys, xxx_todo_changeme):
    ...
    polys[:, :, 0] = np.clip(
        polys[:, :, 0], 0, w - 1
    )  # x coord not max w-1, and not min 0
    polys[:, :, 1] = np.clip(
        polys[:, :, 1], 0, h - 1
    )  # y coord not max h-1, and not min 0
...
def generate_rbox(im_size, text_polys, text_tags, training_mask, shrink_ratio):
    ...
    for i, (poly, tag) in enumerate(zip(text_polys, text_tags)):
        try:
            ...
            d_i = (
                cv2.contourArea(poly) * (1 - shrink_ratio) / cv2.arcLength(poly, True)
                + 0.5
            )
...
def image_label(
    im: np.ndarray,
    text_polys: np.ndarray,
    text_tags: list,
    input_size: int = 640,
    shrink_ratio: float = 0.5,
    degrees: int = 10,
    train: bool = True,
    scales: np.ndarray = np.array([0.5, 1, 2.0, 3.0]),
) -> tuple:
    """
    读取图片并生成label
    :param im: 图片
    ...
    for i in (1, shrink_ratio):
        score_map, training_mask = generate_rbox(
            (h, w), text_polys, text_tags, training_mask, i
        )
        score_maps.append(score_map)
    score_maps = np.array(score_maps, dtype=np.float32)
    imgs = data_aug.random_crop(
        [im, score_maps.transpose((1, 2, 0)), training_mask], (input_size, input_size)
    )
    return (
        imgs[0],
        imgs[1].transpose((2, 0, 1)),
        imgs[2],
    )  # im,score_maps,training_mask#
