...
class Trainer(BaseTrainer):
    def __init__(
        ...
        super(Trainer, self).__init__(
            args, config, model, criterion, metric, weights_init
        )
        ...
        self.logger.info(
            "val dataset has {} samples,{} in dataloader".format(
                self.val_loader.dataset_len, self.val_loader_len
            )
        )

        self.logger.info(
            f"number of trainable parameters: {model.trainable_parameters()}"
        )
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (images, labels, training_masks) in enumerate(self.train_loader):
            ...
            images, labels, training_masks = (
                images.to(self.device),
                labels.to(self.device),
                training_masks.to(self.device),
            )
            ...
            # acc iou
            score_text = cal_text_score(
                preds[:, 0, :, :],
                labels[:, 0, :, :],
                training_masks,
                running_metric_text,
            )
            score_kernel = cal_kernel_score(
                preds[:, 1, :, :],
                labels[:, 1, :, :],
                labels[:, 0, :, :],
                training_masks,
            ...
            self.writer.add_scalar(
                "TRAIN/ACC_IOU/iou_kernel", iou_kernel, self.global_step
            )
    ...
    def _on_epoch_finish(self):
        ...
        if self.epoch_result["epoch"] % self.save_interval == 0:
            net_save_path = (
                f"{self.checkpoint_dir}/PANNet_{self.epoch_result['epoch']}.pth"
            )
            save_best = False
